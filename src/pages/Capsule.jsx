...
const Capsule = () => {
  ...
  const onFilterPastCapsule = async () => {
    await dispatch(fetchPastCapsule({ status: pastCapsuleStatus, type: pastCapsuleType, original_launch: pastCapsule_launch }))
  }
      ...
      <div className="w-[95%] xl:w-[85%] m-auto">
        ...
        <>
          {updatedUpComingCapsule().length > 0 ? <>
            ...
            <div className="grid grid-cols-2 max-[300px]:grid-cols-1 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 2xl:grid-cols-5 p-2 gap-3">
              {
                updatedUpComingCapsule().map((item, idx) => <CapsuleItem key={idx} item={item} onClick={() => showModal(item?.capsule_serial)} />)
              }
            </div>
          </> : null}
        </>
