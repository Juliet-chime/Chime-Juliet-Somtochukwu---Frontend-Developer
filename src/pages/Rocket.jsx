...
const Rocket = () => {
  ...
  return (
    <>
      <CustomModal open={open} onClose={() => setOpen(false)} modalBody={<RocketModal id={rocketId} />} />
              ...
              {rocket?.loading ? (
                ...
                <div>
                  <div className="grid grid-cols-2 max-[300px]:grid-cols-1 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 p-2 gap-3">
                    {allRocket.map((item, idx) => (
                      <RocketItem key={idx} item={item} onClick={() => showModal(item?.rocket_id)} />
                    ))}
                  </div>
                  <Pagination
